...
```{r preprocess}
seed <- 123
>>>>>>> bdee6952f447c6fb3cc6c31d3e2f7fda132107a2
#Ingest
...
## Modeling Validation and Performance
...
#ROC
LR_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$LR)))
LDA_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$LDA)))
PLR_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$PLR)))
NSC_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$NSC)))
nnet_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$nnet)))
svm_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$svm)))
knn_roc_pred <- suppressWarnings(roc(test_results$obs, as.numeric(test_results$knn)))
...
The KNN model performs the best on the test data. This model is selected as the optimal mdoel
...
plot(varImp(knn_model, scale = FALSE), main = "LDA Variable Importance")
