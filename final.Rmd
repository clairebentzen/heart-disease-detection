...
nnet_ImpVar <- varImp(nn_model, scale = FALSE)
top5_nnet_ImpVar <- nnet_ImpVar$importance |> 
  arrange(desc(Overall)) |> slice(1:5)

svm_ImpVar <- varImp(svm_model, scale = FALSE)
svm_ImpVar_df <- as.data.frame(svm_ImpVar$importance)
svm_ImpVar_df$Variable <- rownames(svm_ImpVar$importance) 
svm_ImpVar_summary <- svm_ImpVar_df %>%
     rowwise() %>%
     mutate(Overall = mean(c_across(starts_with('X')), na.rm = TRUE)) %>%
     ungroup()
top5_svm_ImpVar <- svm_ImpVar_summary %>%
     arrange(desc(Overall)) %>% slice(1:5) %>%
     select(Variable, Overall)

knn_ImpVar <- varImp(knn_model, scale = FALSE)
knn_ImpVar_df <- as.data.frame(knn_ImpVar$importance)
knn_ImpVar_df$Variable <- rownames(knn_ImpVar$importance) 
knn_ImpVar_summary <- knn_ImpVar_df %>%
     rowwise() %>%
     mutate(Overall = mean(c_across(starts_with('X')), na.rm = TRUE)) %>%
     ungroup()
top5_knn_ImpVar <- knn_ImpVar_summary %>%
     arrange(desc(Overall)) %>% slice(1:5) %>%
     select(Variable, Overall)
...
top5_ImpVar <- data.frame(
  ...
  nnet_ImpVar = rownames(top5_nnet_ImpVar),
  svm_ImpVar = top5_svm_ImpVar$Variable,
  knn_ImpVar = top5_knn_ImpVar$Variable
)
top5_ImpVar
```
